...
def load_checkpoint(path: str, device: str = "cpu"):
    ...
    cfg = ModelConfig(**json.loads(ck["config"])) if "config" in ck \
        else ModelConfig()
...
def build_input(evidence: EvidenceGrid) -> "np.ndarray":
    ...
    gray = (d["gray"].astype(np.float32) / 255.0 if "gray" in d.files
            else np.zeros_like(occ))
    nrm = (np.abs(d["nrm_abs"].astype(np.float32))
           if "nrm_abs" in d.files else np.zeros((3, x, y, z), np.float32))
    dens = (d["log_density"].astype(np.float32)
            if "log_density" in d.files else np.zeros_like(occ))
    return np.concatenate([occ[None], gray[None], nrm, dens[None]])


def run(evidence: EvidenceGrid, ckpt_path: str, out_npz: str,
        device: str = "cpu", node_threshold: float = 0.5,
        edge_threshold: float = 0.5) -> PatchGraph:
    import torch

    model, cfg = load_checkpoint(ckpt_path, device)
    x = torch.from_numpy(build_input(evidence))[None].to(device)
    pad = [(cfg_pool - s % cfg_pool) % cfg_pool
           for s, cfg_pool in zip(x.shape[2:], [model.cfg.attn_pool] * 3)]
    if any(pad):
        x = torch.nn.functional.pad(
            x, (0, pad[2], 0, pad[1], 0, pad[0]))
    with torch.no_grad():
    ...
    return PatchGraph(npz_path=out_npz, vox_m=evidence.vox_m,
                      origin=evidence.origin, shape=evidence.shape,
