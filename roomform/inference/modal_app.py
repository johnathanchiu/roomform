...
@app.function(image=image, gpu="L4", timeout=1800,
              volumes={VOL: volume})
def infer(evidence_npz: str, ckpt: str, vox_m: float,
    ...
    evidence = EvidenceGrid(
        npz_path=f"{VOL}/{evidence_npz}", vox_m=vox_m, origin=origin,
        shape=shape, visibility_source="synthesized")
...
def main(evidence: str, ckpt: str, vox_m: float = 0.08):
    ...
    out = infer.remote(f"in/{evidence.split('/')[-1]}",
                       f"ckpt/{ckpt.split('/')[-1]}",
                       vox_m, (0.0, 0.0, 0.0), shape)
