...
def complete_shell(evidence: EvidenceGrid, model_id: str = "",
                   ) -> PatchGraph:
...
def load_patchgraph(npz_path: str, vox_m: float,
                    origin: tuple[float, float, float],
                    shape: tuple[int, int, int],
                    model_id: str = "archived") -> PatchGraph:
    """Wrap an exported prediction npz (node_probs [3,...],
    edge_probs [13,...]) as the contract document."""
    return PatchGraph(npz_path=npz_path, vox_m=vox_m, origin=origin,
                      shape=shape, model_id=model_id)
